...
---
= Atualize a partir de uma versão anterior do SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: A atualização para o SCV 6.2 é compatível apenas com o VMware vCenter Server 7 update 1 e versões posteriores, para versões do VMware vCenter Server anteriores à versão 7 update 1, você deve continuar usando o SCV 4.7. A atualização é disruptiva em versões não compatíveis do VMware vCenter Server.
...
CAUTION: Antes de atualizar, verifique se seu ambiente atende aos requisitos do sistema. Consulte link:scpivs44_deployment_planning_and_requirements.html["SnapCenter plug-in para VMware vSphere planejamento e requisitos de implantação"] para obter detalhes.
...
Para obter as informações mais recentes sobre as versões compatíveis, consulte link:https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["Ferramenta de Matriz de interoperabilidade NetApp (IMT)"^].
...
+
.. Faça login no link:https://mysupport.netapp.com/products/index.html["Site de suporte da NetApp"^].
