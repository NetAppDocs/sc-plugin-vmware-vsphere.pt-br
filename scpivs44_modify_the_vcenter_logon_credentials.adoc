---
sidebar: sidebar 
permalink: scpivs44_modify_the_vcenter_logon_credentials.html 
keywords: register SCV with vCenter, re-register SCV, vCenter credentials, vCenter logon credentials, SnapCenter Plug-in for VMware vSphere configuration 
summary: 'Atualize as credenciais de logon do vCenter usadas pelo SnapCenter Plug-in para VMware vSphere (SCV). Use esta tarefa após uma alteração de credencial do vCenter e, para backups vVol, registre novamente as ONTAP tools for VMware vSphere.' 
---
= Modifique as credenciais de logon do vCenter para o SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Use esta tarefa para atualizar as credenciais de logon do vCenter configuradas no SnapCenter Plug-in for VMware vSphere. Essas credenciais são usadas para acessar o vCenter. Se você precisar registrar ou registrar novamente o SCV no vCenter após uma alteração de credencial, conclua esta tarefa. Se a senha do vCenter for alterada, cancele o registro das ONTAP tools for VMware vSphere e, em seguida, registre novamente as ONTAP tools com a nova senha para que os backups vVol continuem funcionando.

.Antes de começar
Você deve saber o endereço IP e as credenciais de logon para a interface de usuário de gerenciamento do SnapCenter Plug-in for VMware vSphere .  Você também deve anotar o token MFA gerado pelo console de manutenção.

* O endereço IP foi exibido quando o plug-in do SnapCenter para VMware vSphere foi implantado.
* Use as credenciais de login fornecidas durante a implantação do plug-in do SnapCenter para VMware vSphere ou conforme modificado posteriormente.
* Gere um token MFA de 6 dígitos usando as opções de configuração do sistema do console de manutenção.


.Passos
. Efetue login na interface de usuário de gerenciamento do SnapCenter Plug-in for VMware vSphere .
+
Utilize o formato `\https://<appliance-IP-address>:8080`

. No painel de navegação esquerdo, selecione *Configuração*.
+
image:scpivs44_image30.png["SnapCenter Plug-in for VMware vSphere"]

. Na página *Configuration*, na seção *vCenter*, selecione *Edit*.
. Introduza a nova palavra-passe e, em seguida, selecione *Guardar*.
+
Não modifique o número da porta.


