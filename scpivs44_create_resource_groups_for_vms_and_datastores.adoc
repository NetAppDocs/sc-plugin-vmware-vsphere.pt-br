...
* Para volumes FlexGroup em ambientes NetApp MetroCluster, se você estiver usando ONTAP 9.8 ou 9.9, reinicie o serviço SnapCenter Plug-in for VMware vSphere e resincronize os relacionamentos SnapMirror após um switchover ou switchback antes de fazer backup dos grupos de recursos. No ONTAP 9.8, os backups podem travar após um switchback; isso foi resolvido no ONTAP 9.9.
