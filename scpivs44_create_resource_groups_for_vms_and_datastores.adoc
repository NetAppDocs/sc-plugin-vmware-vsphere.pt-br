...
NOTE: O sistema de armazenamento ASA R2 não suporta a renomeação de instantâneos e, como resultado, o backup de renomeação da SCV e os recursos de nomeação de instantâneo _recentes não são suportados.
...
. Revise o resumo e clique em *Finish*. A partir da proteção secundária da VCR 6,1 para os recursos dos sistemas ASA R2 estão visíveis na página de resumo.
+
